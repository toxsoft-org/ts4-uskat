...
public class NonceUtil {
  ...
  static {
    ...
    new Thread( () -> {
      ...
      LoggerFactory.getLogger( NonceUtil.class ).info( "SecureRandom seeded in {}ms.",
   ...
   * <p>
   * The length is determined by the policy: see {@link #getNonceLength(SecurityPolicy)}. <b>Important</b>: this should
   * only be used to generate a nonce exchanged when establishing a secure channel. The nonce used by sessions has
   ...
   * <p>
   * <b>Important</b>: this should only be used to validate the nonce exchanged when establishing a secure channel. The
   * nonce used by sessions has different length requirements.
   *
   ...
   * @throws UaException if nonce validation failed.
   */
  public static void validateNonce( ByteString nonce )
      throws UaException {
   ...
   * @throws UaException if nonce validation failed.
   */
  public static void validateNonce( ByteString nonce, int minimumLength )
      throws UaException {
    byte[] bs = nonce.bytesOrEmpty();

    if( bs.length < minimumLength ) {
      throw new UaException( StatusCodes.Bad_NonceInvalid, "nonce must be at least " + minimumLength + " bytes" );
    }
