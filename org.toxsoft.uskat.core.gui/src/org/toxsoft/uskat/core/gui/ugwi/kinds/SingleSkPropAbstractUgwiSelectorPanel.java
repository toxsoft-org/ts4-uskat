 ...
 * @author vs
 */
public class SingleSkPropAbstractUgwiSelectorPanel
   ...
   */
  public SingleSkPropAbstractUgwiSelectorPanel( ITsGuiContext aContext, boolean aIsViewer ) {
  ...
  protected Control doCreateControl( Composite aParent ) {
    ...
    SashForm horizontalSashForm = new SashForm( board, SWT.HORIZONTAL );

    // panels
    panelClasses.createControl( horizontalSashForm );
    panelProps.createControl( horizontalSashForm );
    // setup
  ...
  public Ugwi selectedItem() {
    ...
    ISkClassInfo clsInfo = panelClasses.selectedItem();
    String ugwiKindId = tsContext().params().getStr( OPDEF_SK_UGWI_KIND_ID );
    return switch( ugwiKindId ) {
      case UgwiKindSkRtDataInfo.KIND_ID -> UgwiKindSkRtDataInfo.makeUgwi( clsInfo.id(), selProp.id() );
  ...
  private void whenClassSelectionChanges() {
    ...
    if( cinf != null ) {
      ...
      propItemsProvider.items().setAll( cinf.props( getClassPropKind() ).list() );
  ...
  public static Ugwi selectUgwi( ITsDialogInfo aDialogInfo, Ugwi aInitVal, IdChain aSkConnKey ) {
    ...
    -> new TsDialogGenericEntityEditPanel<>( par, od, ( aContext, aViewer ) -> {
      SingleSkPropAbstractUgwiSelectorPanel panel = new SingleSkPropAbstractUgwiSelectorPanel( aContext, aViewer );
      return panel;
    } );
