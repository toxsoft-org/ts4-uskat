...
import static org.toxsoft.uskat.sysext.realtime.supports.histdata.IS5HistDataHardConstants.*;
import static org.toxsoft.uskat.sysext.realtime.supports.histdata.sequences.IS5Resources.*;
...
public final class S5HistDataSequenceFactory
  ...
  public IParameterized doTypeInfo( Gwid aGwid ) {
    ...
    ISkRtdataInfo rtdataInfo = classInfo.rtdInfos().getByKey( dataId );
    if( !rtdataInfo.isHist() ) {
      // Данное не является хранимым
      throw new TsIllegalArgumentRtException( ERR_NO_HISTDATA, aGwid );
    }
    // Идентификатор типа данного
    String typeId = rtdataInfo.typeId();
    // Атомарный тип данного
    EAtomicType atomicType = sysdescrReader().getType( typeId ).atomicType();
    ...
    IAvMetaConstants.DDEF_DEFAULT_VALUE.setValue( params, rtdataInfo.dataDef().defaultValue() );
    IDpuHardConstants.OP_DATA_TYPE_ID.setValue( params, avStr( typeId ) );
  ...
  public Object doCreateValueArray( IParameterized aTypeInfo, int aSize ) {
    String typeId = IDpuHardConstants.OP_DATA_TYPE_ID.getValue( aTypeInfo.params() ).asString();
    EAtomicType atomicType = sysdescrReader().getType( typeId ).atomicType();
    switch( atomicType ) {
      case BOOLEAN:
  ...
  public Object doGetSyncDefaultValue( IParameterized aTypeInfo ) {
    String typeId = IDpuHardConstants.OP_DATA_TYPE_ID.getValue( aTypeInfo.params() ).asString();
    IDataDef type = sysdescrReader().getType( typeId );
