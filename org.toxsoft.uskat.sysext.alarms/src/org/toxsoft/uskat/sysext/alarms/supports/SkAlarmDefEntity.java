...
import static org.toxsoft.uskat.sysext.alarms.supports.SkAlarmEntitiesUtils.*;
 ...
 * @author mvk
 */
@NamedQueries( { @NamedQuery( name = QUERY_GET, query = "SELECT alarmDef FROM SkAlarmDefEntity alarmDef" ), } )
@Entity
public class SkAlarmDefEntity
   ...
   */
  public SkAlarmDefEntity( String aId, String aMessage ) {
    TsNullArgumentRtException.checkNulls( aId, aMessage );
    id = StridUtils.checkValidIdPath( aId );
    setPriority( EAlarmPriority.NORMAL );
    setName( TsLibUtils.EMPTY_STRING );
    setDescription( TsLibUtils.EMPTY_STRING );
    setMessage( aMessage );
  }
   ...
   */
  public SkAlarmDefEntity( ISkAlarmDef aSkAlarmDef ) {
