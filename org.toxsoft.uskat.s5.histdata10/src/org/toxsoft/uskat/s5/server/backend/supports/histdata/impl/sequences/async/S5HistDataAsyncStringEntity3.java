 ...
 */
@Entity
public class S5HistDataAsyncStringEntity3
    extends S5HistDataAsyncBlock<String[], S5HistDataAsyncStringBlobEntity3> {
   ...
   * @return {@link S5HistDataAsyncStringEntity3} блок значений
   ...
   */
  static S5HistDataAsyncStringEntity3 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncStringBlobEntity3 blob = new S5HistDataAsyncStringBlobEntity3( timestamps, values );
    return new S5HistDataAsyncStringEntity3( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncStringEntity3() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncStringBlobEntity3} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncStringEntity3( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncStringBlobEntity3 aBlob ) {
   ...
   */
  S5HistDataAsyncStringEntity3( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected IS5SequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      String[] aValues ) {
    S5HistDataAsyncStringBlobEntity3 blob = new S5HistDataAsyncStringBlobEntity3( aTimestamps, aValues );
    return new S5HistDataAsyncStringEntity3( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncStringBlobEntity3 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncStringBlobEntity3( aResultSet );
  }
