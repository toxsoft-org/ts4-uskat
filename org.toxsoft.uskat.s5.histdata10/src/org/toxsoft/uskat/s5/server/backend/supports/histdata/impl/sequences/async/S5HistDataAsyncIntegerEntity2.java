 ...
 */
@Entity
public class S5HistDataAsyncIntegerEntity2
    extends S5HistDataAsyncBlock<long[], S5HistDataAsyncIntegerBlobEntity2> {
   ...
   * @return {@link S5HistDataAsyncIntegerEntity2} блок значений
   ...
   */
  static S5HistDataAsyncIntegerEntity2 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncIntegerBlobEntity2 blob = new S5HistDataAsyncIntegerBlobEntity2( timestamps, values );
    return new S5HistDataAsyncIntegerEntity2( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncIntegerEntity2() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncIntegerBlobEntity2} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncIntegerEntity2( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncIntegerBlobEntity2 aBlob ) {
   ...
   */
  S5HistDataAsyncIntegerEntity2( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected IS5SequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      long[] aValues ) {
    S5HistDataAsyncIntegerBlobEntity2 blob = new S5HistDataAsyncIntegerBlobEntity2( aTimestamps, aValues );
    return new S5HistDataAsyncIntegerEntity2( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncIntegerBlobEntity2 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncIntegerBlobEntity2( aResultSet );
  }
