 ...
 */
@Entity
public class S5HistDataAsyncFloatingEntity8
    extends S5HistDataAsyncBlock<double[], S5HistDataAsyncFloatingBlobEntity8> {
   ...
   * @return {@link S5HistDataAsyncFloatingEntity8} блок значений
   ...
   */
  static S5HistDataAsyncFloatingEntity8 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncFloatingBlobEntity8 blob = new S5HistDataAsyncFloatingBlobEntity8( timestamps, values );
    return new S5HistDataAsyncFloatingEntity8( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncFloatingEntity8() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncFloatingBlobEntity8} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncFloatingEntity8( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncFloatingBlobEntity8 aBlob ) {
   ...
   */
  S5HistDataAsyncFloatingEntity8( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      double[] aValues ) {
    S5HistDataAsyncFloatingBlobEntity8 blob = new S5HistDataAsyncFloatingBlobEntity8( aTimestamps, aValues );
    return new S5HistDataAsyncFloatingEntity8( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncFloatingBlobEntity8 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncFloatingBlobEntity8( aResultSet );
  }
