 ...
 */
@Entity
public class S5HistDataAsyncValobjEntity4
    extends S5HistDataAsyncBlock<String[], S5HistDataAsyncValobjBlobEntity4> {
   ...
   * @return {@link S5HistDataAsyncValobjEntity4} блок значений
   ...
   */
  static S5HistDataAsyncValobjEntity4 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncValobjBlobEntity4 blob = new S5HistDataAsyncValobjBlobEntity4( timestamps, values );
    return new S5HistDataAsyncValobjEntity4( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncValobjEntity4() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncValobjBlobEntity4} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncValobjEntity4( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncValobjBlobEntity4 aBlob ) {
   ...
   */
  S5HistDataAsyncValobjEntity4( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected IS5SequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      String[] aValues ) {
    S5HistDataAsyncValobjBlobEntity4 blob = new S5HistDataAsyncValobjBlobEntity4( aTimestamps, aValues );
    return new S5HistDataAsyncValobjEntity4( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncValobjBlobEntity4 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncValobjBlobEntity4( aResultSet );
  }
