 ...
 */
@Entity
public class S5HistDataAsyncIntegerEntity8
    extends S5HistDataAsyncBlock<long[], S5HistDataAsyncIntegerBlobEntity8> {
   ...
   * @return {@link S5HistDataAsyncIntegerEntity8} блок значений
   ...
   */
  static S5HistDataAsyncIntegerEntity8 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncIntegerBlobEntity8 blob = new S5HistDataAsyncIntegerBlobEntity8( timestamps, values );
    return new S5HistDataAsyncIntegerEntity8( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncIntegerEntity8() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncIntegerBlobEntity8} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncIntegerEntity8( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncIntegerBlobEntity8 aBlob ) {
   ...
   */
  S5HistDataAsyncIntegerEntity8( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      long[] aValues ) {
    S5HistDataAsyncIntegerBlobEntity8 blob = new S5HistDataAsyncIntegerBlobEntity8( aTimestamps, aValues );
    return new S5HistDataAsyncIntegerEntity8( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncIntegerBlobEntity8 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncIntegerBlobEntity8( aResultSet );
  }
