 ...
 */
@Entity
public class S5HistDataAsyncIntegerEntity6
    extends S5HistDataAsyncBlock<long[], S5HistDataAsyncIntegerBlobEntity6> {
   ...
   * @return {@link S5HistDataAsyncIntegerEntity6} блок значений
   ...
   */
  static S5HistDataAsyncIntegerEntity6 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncIntegerBlobEntity6 blob = new S5HistDataAsyncIntegerBlobEntity6( timestamps, values );
    return new S5HistDataAsyncIntegerEntity6( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncIntegerEntity6() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncIntegerBlobEntity6} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncIntegerEntity6( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncIntegerBlobEntity6 aBlob ) {
   ...
   */
  S5HistDataAsyncIntegerEntity6( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected IS5SequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      long[] aValues ) {
    S5HistDataAsyncIntegerBlobEntity6 blob = new S5HistDataAsyncIntegerBlobEntity6( aTimestamps, aValues );
    return new S5HistDataAsyncIntegerEntity6( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncIntegerBlobEntity6 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncIntegerBlobEntity6( aResultSet );
  }
