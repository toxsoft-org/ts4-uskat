 ...
 */
@Entity
public class S5HistDataAsyncFloatingEntity1
    extends S5HistDataAsyncBlock<double[], S5HistDataAsyncFloatingBlobEntity1> {
   ...
   * @return {@link S5HistDataAsyncFloatingEntity1} блок значений
   ...
   */
  static S5HistDataAsyncFloatingEntity1 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncFloatingBlobEntity1 blob = new S5HistDataAsyncFloatingBlobEntity1( timestamps, values );
    return new S5HistDataAsyncFloatingEntity1( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncFloatingEntity1() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncFloatingBlobEntity1} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncFloatingEntity1( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncFloatingBlobEntity1 aBlob ) {
   ...
   */
  S5HistDataAsyncFloatingEntity1( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      double[] aValues ) {
    S5HistDataAsyncFloatingBlobEntity1 blob = new S5HistDataAsyncFloatingBlobEntity1( aTimestamps, aValues );
    return new S5HistDataAsyncFloatingEntity1( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncFloatingBlobEntity1 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncFloatingBlobEntity1( aResultSet );
  }
