 ...
 */
@Entity
public class S5HistDataAsyncIntegerEntity3
    extends S5HistDataAsyncBlock<long[], S5HistDataAsyncIntegerBlobEntity3> {
   ...
   * @return {@link S5HistDataAsyncIntegerEntity3} блок значений
   ...
   */
  static S5HistDataAsyncIntegerEntity3 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncIntegerBlobEntity3 blob = new S5HistDataAsyncIntegerBlobEntity3( timestamps, values );
    return new S5HistDataAsyncIntegerEntity3( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncIntegerEntity3() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncIntegerBlobEntity3} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncIntegerEntity3( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncIntegerBlobEntity3 aBlob ) {
   ...
   */
  S5HistDataAsyncIntegerEntity3( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected IS5SequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      long[] aValues ) {
    S5HistDataAsyncIntegerBlobEntity3 blob = new S5HistDataAsyncIntegerBlobEntity3( aTimestamps, aValues );
    return new S5HistDataAsyncIntegerEntity3( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncIntegerBlobEntity3 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncIntegerBlobEntity3( aResultSet );
  }
