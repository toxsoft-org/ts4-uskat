 ...
 */
@Entity
public class S5HistDataAsyncValobjEntity3
    extends S5HistDataAsyncBlock<String[], S5HistDataAsyncValobjBlobEntity3> {
   ...
   * @return {@link S5HistDataAsyncValobjEntity3} блок значений
   ...
   */
  static S5HistDataAsyncValobjEntity3 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncValobjBlobEntity3 blob = new S5HistDataAsyncValobjBlobEntity3( timestamps, values );
    return new S5HistDataAsyncValobjEntity3( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncValobjEntity3() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncValobjBlobEntity3} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncValobjEntity3( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncValobjBlobEntity3 aBlob ) {
   ...
   */
  S5HistDataAsyncValobjEntity3( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected IS5SequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      String[] aValues ) {
    S5HistDataAsyncValobjBlobEntity3 blob = new S5HistDataAsyncValobjBlobEntity3( aTimestamps, aValues );
    return new S5HistDataAsyncValobjEntity3( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncValobjBlobEntity3 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncValobjBlobEntity3( aResultSet );
  }
