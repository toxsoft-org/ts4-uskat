 ...
 */
@Entity
public class S5HistDataAsyncIntegerEntity4
    extends S5HistDataAsyncBlock<long[], S5HistDataAsyncIntegerBlobEntity4> {
   ...
   * @return {@link S5HistDataAsyncIntegerEntity4} блок значений
   ...
   */
  static S5HistDataAsyncIntegerEntity4 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncIntegerBlobEntity4 blob = new S5HistDataAsyncIntegerBlobEntity4( timestamps, values );
    return new S5HistDataAsyncIntegerEntity4( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncIntegerEntity4() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncIntegerBlobEntity4} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncIntegerEntity4( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncIntegerBlobEntity4 aBlob ) {
   ...
   */
  S5HistDataAsyncIntegerEntity4( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected IS5SequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      long[] aValues ) {
    S5HistDataAsyncIntegerBlobEntity4 blob = new S5HistDataAsyncIntegerBlobEntity4( aTimestamps, aValues );
    return new S5HistDataAsyncIntegerEntity4( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncIntegerBlobEntity4 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncIntegerBlobEntity4( aResultSet );
  }
