 ...
 */
@Entity
public class S5HistDataAsyncFloatingEntity9
    extends S5HistDataAsyncBlock<double[], S5HistDataAsyncFloatingBlobEntity9> {
   ...
   * @return {@link S5HistDataAsyncFloatingEntity9} блок значений
   ...
   */
  static S5HistDataAsyncFloatingEntity9 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncFloatingBlobEntity9 blob = new S5HistDataAsyncFloatingBlobEntity9( timestamps, values );
    return new S5HistDataAsyncFloatingEntity9( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncFloatingEntity9() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncFloatingBlobEntity9} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncFloatingEntity9( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncFloatingBlobEntity9 aBlob ) {
   ...
   */
  S5HistDataAsyncFloatingEntity9( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected IS5SequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      double[] aValues ) {
    S5HistDataAsyncFloatingBlobEntity9 blob = new S5HistDataAsyncFloatingBlobEntity9( aTimestamps, aValues );
    return new S5HistDataAsyncFloatingEntity9( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncFloatingBlobEntity9 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncFloatingBlobEntity9( aResultSet );
  }
