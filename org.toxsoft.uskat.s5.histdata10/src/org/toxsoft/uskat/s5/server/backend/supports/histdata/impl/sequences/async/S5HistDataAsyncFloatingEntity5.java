 ...
 */
@Entity
public class S5HistDataAsyncFloatingEntity5
    extends S5HistDataAsyncBlock<double[], S5HistDataAsyncFloatingBlobEntity5> {
   ...
   * @return {@link S5HistDataAsyncFloatingEntity5} блок значений
   ...
   */
  static S5HistDataAsyncFloatingEntity5 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncFloatingBlobEntity5 blob = new S5HistDataAsyncFloatingBlobEntity5( timestamps, values );
    return new S5HistDataAsyncFloatingEntity5( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncFloatingEntity5() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncFloatingBlobEntity5} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncFloatingEntity5( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncFloatingBlobEntity5 aBlob ) {
   ...
   */
  S5HistDataAsyncFloatingEntity5( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      double[] aValues ) {
    S5HistDataAsyncFloatingBlobEntity5 blob = new S5HistDataAsyncFloatingBlobEntity5( aTimestamps, aValues );
    return new S5HistDataAsyncFloatingEntity5( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncFloatingBlobEntity5 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncFloatingBlobEntity5( aResultSet );
  }
