 ...
 */
@Entity
public class S5HistDataAsyncValobjEntity5
    extends S5HistDataAsyncBlock<String[], S5HistDataAsyncValobjBlobEntity5> {
   ...
   * @return {@link S5HistDataAsyncValobjEntity5} блок значений
   ...
   */
  static S5HistDataAsyncValobjEntity5 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncValobjBlobEntity5 blob = new S5HistDataAsyncValobjBlobEntity5( timestamps, values );
    return new S5HistDataAsyncValobjEntity5( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncValobjEntity5() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncValobjBlobEntity5} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncValobjEntity5( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncValobjBlobEntity5 aBlob ) {
   ...
   */
  S5HistDataAsyncValobjEntity5( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      String[] aValues ) {
    S5HistDataAsyncValobjBlobEntity5 blob = new S5HistDataAsyncValobjBlobEntity5( aTimestamps, aValues );
    return new S5HistDataAsyncValobjEntity5( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncValobjBlobEntity5 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncValobjBlobEntity5( aResultSet );
  }
