 ...
 */
@Entity
public class S5HistDataAsyncIntegerEntity9
    extends S5HistDataAsyncBlock<long[], S5HistDataAsyncIntegerBlobEntity9> {
   ...
   * @return {@link S5HistDataAsyncIntegerEntity9} блок значений
   ...
   */
  static S5HistDataAsyncIntegerEntity9 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncIntegerBlobEntity9 blob = new S5HistDataAsyncIntegerBlobEntity9( timestamps, values );
    return new S5HistDataAsyncIntegerEntity9( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncIntegerEntity9() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncIntegerBlobEntity9} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncIntegerEntity9( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncIntegerBlobEntity9 aBlob ) {
   ...
   */
  S5HistDataAsyncIntegerEntity9( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected IS5SequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      long[] aValues ) {
    S5HistDataAsyncIntegerBlobEntity9 blob = new S5HistDataAsyncIntegerBlobEntity9( aTimestamps, aValues );
    return new S5HistDataAsyncIntegerEntity9( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncIntegerBlobEntity9 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncIntegerBlobEntity9( aResultSet );
  }
