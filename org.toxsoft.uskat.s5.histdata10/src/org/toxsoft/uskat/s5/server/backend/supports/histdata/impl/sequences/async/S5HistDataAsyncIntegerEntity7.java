 ...
 */
@Entity
public class S5HistDataAsyncIntegerEntity7
    extends S5HistDataAsyncBlock<long[], S5HistDataAsyncIntegerBlobEntity7> {
   ...
   * @return {@link S5HistDataAsyncIntegerEntity7} блок значений
   ...
   */
  static S5HistDataAsyncIntegerEntity7 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncIntegerBlobEntity7 blob = new S5HistDataAsyncIntegerBlobEntity7( timestamps, values );
    return new S5HistDataAsyncIntegerEntity7( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncIntegerEntity7() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncIntegerBlobEntity7} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncIntegerEntity7( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncIntegerBlobEntity7 aBlob ) {
   ...
   */
  S5HistDataAsyncIntegerEntity7( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected IS5SequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      long[] aValues ) {
    S5HistDataAsyncIntegerBlobEntity7 blob = new S5HistDataAsyncIntegerBlobEntity7( aTimestamps, aValues );
    return new S5HistDataAsyncIntegerEntity7( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncIntegerBlobEntity7 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncIntegerBlobEntity7( aResultSet );
  }
