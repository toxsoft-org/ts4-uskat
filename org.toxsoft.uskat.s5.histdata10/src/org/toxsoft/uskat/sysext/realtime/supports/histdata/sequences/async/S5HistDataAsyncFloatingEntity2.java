 ...
 */
@Entity
public class S5HistDataAsyncFloatingEntity2
    extends S5HistDataAsyncBlock<double[], S5HistDataAsyncFloatingBlobEntity2> {
   ...
   * @return {@link S5HistDataAsyncFloatingEntity2} блок значений
   ...
   */
  static S5HistDataAsyncFloatingEntity2 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncFloatingBlobEntity2 blob = new S5HistDataAsyncFloatingBlobEntity2( timestamps, values );
    return new S5HistDataAsyncFloatingEntity2( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncFloatingEntity2() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncFloatingBlobEntity2} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncFloatingEntity2( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncFloatingBlobEntity2 aBlob ) {
   ...
   */
  S5HistDataAsyncFloatingEntity2( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      double[] aValues ) {
    S5HistDataAsyncFloatingBlobEntity2 blob = new S5HistDataAsyncFloatingBlobEntity2( aTimestamps, aValues );
    return new S5HistDataAsyncFloatingEntity2( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncFloatingBlobEntity2 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncFloatingBlobEntity2( aResultSet );
  }
