 ...
 */
@Entity
public class S5HistDataAsyncFloatingEntity6
    extends S5HistDataAsyncBlock<double[], S5HistDataAsyncFloatingBlobEntity6> {
   ...
   * @return {@link S5HistDataAsyncFloatingEntity6} блок значений
   ...
   */
  static S5HistDataAsyncFloatingEntity6 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncFloatingBlobEntity6 blob = new S5HistDataAsyncFloatingBlobEntity6( timestamps, values );
    return new S5HistDataAsyncFloatingEntity6( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncFloatingEntity6() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncFloatingBlobEntity6} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncFloatingEntity6( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncFloatingBlobEntity6 aBlob ) {
   ...
   */
  S5HistDataAsyncFloatingEntity6( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      double[] aValues ) {
    S5HistDataAsyncFloatingBlobEntity6 blob = new S5HistDataAsyncFloatingBlobEntity6( aTimestamps, aValues );
    return new S5HistDataAsyncFloatingEntity6( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncFloatingBlobEntity6 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncFloatingBlobEntity6( aResultSet );
  }
