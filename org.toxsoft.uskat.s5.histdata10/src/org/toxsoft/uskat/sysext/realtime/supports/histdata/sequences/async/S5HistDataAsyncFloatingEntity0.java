 ...
 */
@Entity
public class S5HistDataAsyncFloatingEntity0
    extends S5HistDataAsyncBlock<double[], S5HistDataAsyncFloatingBlobEntity0> {
   ...
   * @return {@link S5HistDataAsyncFloatingEntity0} блок значений
   ...
   */
  static S5HistDataAsyncFloatingEntity0 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncFloatingBlobEntity0 blob = new S5HistDataAsyncFloatingBlobEntity0( timestamps, values );
    return new S5HistDataAsyncFloatingEntity0( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncFloatingEntity0() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncFloatingBlobEntity0} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncFloatingEntity0( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncFloatingBlobEntity0 aBlob ) {
   ...
   */
  S5HistDataAsyncFloatingEntity0( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      double[] aValues ) {
    S5HistDataAsyncFloatingBlobEntity0 blob = new S5HistDataAsyncFloatingBlobEntity0( aTimestamps, aValues );
    return new S5HistDataAsyncFloatingEntity0( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncFloatingBlobEntity0 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncFloatingBlobEntity0( aResultSet );
  }
