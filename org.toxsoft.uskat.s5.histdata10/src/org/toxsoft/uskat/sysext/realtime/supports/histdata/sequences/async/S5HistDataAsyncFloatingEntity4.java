 ...
 */
@Entity
public class S5HistDataAsyncFloatingEntity4
    extends S5HistDataAsyncBlock<double[], S5HistDataAsyncFloatingBlobEntity4> {
   ...
   * @return {@link S5HistDataAsyncFloatingEntity4} блок значений
   ...
   */
  static S5HistDataAsyncFloatingEntity4 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncFloatingBlobEntity4 blob = new S5HistDataAsyncFloatingBlobEntity4( timestamps, values );
    return new S5HistDataAsyncFloatingEntity4( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncFloatingEntity4() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncFloatingBlobEntity4} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncFloatingEntity4( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncFloatingBlobEntity4 aBlob ) {
   ...
   */
  S5HistDataAsyncFloatingEntity4( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      double[] aValues ) {
    S5HistDataAsyncFloatingBlobEntity4 blob = new S5HistDataAsyncFloatingBlobEntity4( aTimestamps, aValues );
    return new S5HistDataAsyncFloatingEntity4( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncFloatingBlobEntity4 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncFloatingBlobEntity4( aResultSet );
  }
