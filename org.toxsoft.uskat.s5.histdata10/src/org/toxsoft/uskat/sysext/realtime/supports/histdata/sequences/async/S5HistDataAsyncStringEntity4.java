 ...
 */
@Entity
public class S5HistDataAsyncStringEntity4
    extends S5HistDataAsyncBlock<String[], S5HistDataAsyncStringBlobEntity4> {
   ...
   * @return {@link S5HistDataAsyncStringEntity4} блок значений
   ...
   */
  static S5HistDataAsyncStringEntity4 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncStringBlobEntity4 blob = new S5HistDataAsyncStringBlobEntity4( timestamps, values );
    return new S5HistDataAsyncStringEntity4( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncStringEntity4() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncStringBlobEntity4} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncStringEntity4( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncStringBlobEntity4 aBlob ) {
   ...
   */
  S5HistDataAsyncStringEntity4( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      String[] aValues ) {
    S5HistDataAsyncStringBlobEntity4 blob = new S5HistDataAsyncStringBlobEntity4( aTimestamps, aValues );
    return new S5HistDataAsyncStringEntity4( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncStringBlobEntity4 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncStringBlobEntity4( aResultSet );
  }
