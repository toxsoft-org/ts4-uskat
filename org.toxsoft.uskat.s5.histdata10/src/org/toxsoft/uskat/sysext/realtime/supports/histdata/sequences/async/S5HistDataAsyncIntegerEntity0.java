 ...
 */
@Entity
public class S5HistDataAsyncIntegerEntity0
    extends S5HistDataAsyncBlock<long[], S5HistDataAsyncIntegerBlobEntity0> {
   ...
   * @return {@link S5HistDataAsyncIntegerEntity0} блок значений
   ...
   */
  static S5HistDataAsyncIntegerEntity0 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncIntegerBlobEntity0 blob = new S5HistDataAsyncIntegerBlobEntity0( timestamps, values );
    return new S5HistDataAsyncIntegerEntity0( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncIntegerEntity0() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncIntegerBlobEntity0} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncIntegerEntity0( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncIntegerBlobEntity0 aBlob ) {
   ...
   */
  S5HistDataAsyncIntegerEntity0( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      long[] aValues ) {
    S5HistDataAsyncIntegerBlobEntity0 blob = new S5HistDataAsyncIntegerBlobEntity0( aTimestamps, aValues );
    return new S5HistDataAsyncIntegerEntity0( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncIntegerBlobEntity0 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncIntegerBlobEntity0( aResultSet );
  }
