 ...
 */
@Entity
public class S5HistDataAsyncFloatingEntity7
    extends S5HistDataAsyncBlock<double[], S5HistDataAsyncFloatingBlobEntity7> {
   ...
   * @return {@link S5HistDataAsyncFloatingEntity7} блок значений
   ...
   */
  static S5HistDataAsyncFloatingEntity7 create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncFloatingBlobEntity7 blob = new S5HistDataAsyncFloatingBlobEntity7( timestamps, values );
    return new S5HistDataAsyncFloatingEntity7( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncFloatingEntity7() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncFloatingBlobEntity7} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncFloatingEntity7( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncFloatingBlobEntity7 aBlob ) {
   ...
   */
  S5HistDataAsyncFloatingEntity7( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long[] aTimestamps,
      double[] aValues ) {
    S5HistDataAsyncFloatingBlobEntity7 blob = new S5HistDataAsyncFloatingBlobEntity7( aTimestamps, aValues );
    return new S5HistDataAsyncFloatingEntity7( aTypeInfo, gwid(), blob );
  }

  @Override
  protected S5HistDataAsyncFloatingBlobEntity7 doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncFloatingBlobEntity7( aResultSet );
  }
