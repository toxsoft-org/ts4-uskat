...
public class DtoCommand
  ...
  private static final long serialVersionUID = 8151096987015893400L;
  ...
  public static final IEntityKeeper<IDtoCommand> KEEPER =
      new AbstractEntityKeeper<>( IDtoCommand.class, EEncloseMode.ENCLOSES_BASE_CLASS, null ) {
        ...
        protected IDtoCommand doRead( IStrioReader aSr ) {
          ...
          return new DtoCommand( timestamp, id, cmdGwid, authorSkid, argValues );
        }
      };
   ...
   * @param aId String - идентификатор (ИД-путь) экземпляра команды
   * @param aCmd {@link Gwid} - команда
   * @param aAuthor {@link Skid} - автор
   * @param aArgs {@link IOptionSet} - значения аргументов
   */
  public DtoCommand( long aTime, String aId, Gwid aCmd, Skid aAuthor, IOptionSet aArgs ) {
    timestamp = aTime;
    id = StridUtils.checkValidIdPath( aId );
    TsNullArgumentRtException.checkNulls( aId, aCmd, aAuthor, aArgs );
    cmdGwid = aCmd;
    authorSkid = aAuthor;
  ...
  public boolean equals( Object aThat ) {
    ...
    if( aThat instanceof IDtoCommand that ) {
      return id.equals( that.id() ) && timestamp == that.timestamp() && cmdGwid.equals( that.cmdGwid() )
