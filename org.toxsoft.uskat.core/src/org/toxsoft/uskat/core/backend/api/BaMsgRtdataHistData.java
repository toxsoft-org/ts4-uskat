...
public class BaMsgRtdataHistData
   ...
   *
   * @param aNewValues {@link IMap}&lt;{@link Gwid},{@link ITimedList}&gt; - map "RTdata GWID" - "historical values"
   * @return {@link GtMessage} - created instance of the message
   * @throws TsNullArgumentRtException any argument = <code>null</code>
   */
  public GtMessage makeMessage( IMap<Gwid, ITimedList<ITemporalAtomicValue>> aNewValues ) {
    ...
    for( int i = 0, count = aNewValues.size(); i < count; i++ ) {
      ...
      ITimedList<ITemporalAtomicValue> v = aNewValues.values().get( i );
   ...
   * @return {@link IMap}&lt;{@link Gwid},{@link ITimedList}&gt; - map "RTdata GWID" - "historical value"
   */
  public IMap<Gwid, ITimedList<ITemporalAtomicValue>> getNewValues( GenericMessage aMsg ) {
    String s = getArg( aMsg, ARGID_NEW_VALUES ).asString();
    IMapEdit<Gwid, ITimedList<ITemporalAtomicValue>> map = new ElemMap<>();
