 ...
 * the specified time interval and then processed. Result of processing is {@link ITimedList} usually of the same value
 ...
 * <ul>
 * <li><b>filtering</b> - some values from the initial {@link ITimedList} may be rejected by the filter. Filter is
 * created based on specified parameters {@link #filterParams()}. For example, filter may reject/accept commands with
 * the specified argument, or atomic values if they are out of the range, etc.;</li>
 * <li>applying an aggregating <b>function</b> - function {@link #funcId()} with arguments {@link #funcArgs()} will be
 * applied to each filtered element. Most function are aggregating functions having fixed time periods called
 ...
 * data list of the specified {@link #dataGwid()}, thus implementing {@link ISkQueryRawHistory}.
 *
 * @author hazard157
 */
public interface IDtoQueryParam {
   ...
   * <p>
   * Created filter must accept elements of type, depending on {@link #dataGwid()}. For example, for
