...
public class UgwiKindSkLink
  ...
  public static class Kind
    ...
    public ISkidList doFindContent( Ugwi aUgwi ) {
      ...
      IStringMap<IDtoLinkFwd> linksMap = ls.getAllLinksFwd( gwid.skid() );
      if( linksMap.hasKey( gwid.propId() ) ) {
        return linksMap.getByKey( gwid.propId() ).rightSkids();
      }
      return null;
    }
  ...
  protected Gwid doGetGwid( Ugwi aUgwi ) {
    ...
    return Gwid.createLink( chain.get( IDX_CLASS_ID ), chain.get( IDX_OBJ_STRID ), chain.get( IDX_LINK_ID ) );
  }
