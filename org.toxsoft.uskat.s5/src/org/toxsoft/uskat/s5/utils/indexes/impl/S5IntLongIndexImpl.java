...
import org.toxsoft.uskat.s5.utils.indexes.IIntLongIndexEdit;

/**
 * Бинарный индекс длинного целочисленного значения по целочисленному ключу
 *
 * @author mvk
 */
class S5IntLongIndexImpl
    extends S5AbstractIndex<Integer, Long>
    implements IIntLongIndexEdit {
   ...
   */
  S5IntLongIndexImpl( boolean aUnique, int[] aKeys, long[] aValues, boolean aRestore ) {
  ...
  @Override
  public void add( IIntList aKeys, ILongList aValues ) {
    TsNullArgumentRtException.checkNulls( aKeys, aValues );
  ...
  @Override
  public void add( int[] aKeys, long[] aValues ) {
    TsNullArgumentRtException.checkNulls( aKeys, aValues );
    checkSize( aKeys.length, aValues.length );
  ...
  public Long value( int aIndex ) {
    ...
    return Long.valueOf( values[aIndex] );
  }

  @Override
  public long longValue( int aIndex ) {
    if( aIndex < 0 || aIndex >= watermark ) {
      Long valueIndex = Long.valueOf( aIndex );
      throw new TsIllegalArgumentRtException( MSG_ERR_INDEX_VALUE_OUT, valueIndex, Long.valueOf( watermark ) );
    }
  ...
  @Override
  public Long findValue( Integer aKey ) {
    return Long.valueOf( findValue( aKey.intValue() ) );
  }

  @Override
  public long findValue( int aKey ) {
    int index = findIndex( aKey );
    if( index < 0 ) {
  ...
  @Override
  public Long getValue( Integer aKey ) {
    return Long.valueOf( getValue( aKey.intValue() ) );
  }
