...
import org.toxsoft.uskat.s5.utils.indexes.ILongBooleanIndexEdit;

/**
 * Бинарный индекс логического значения по длинному целочисленному ключу
 *
 * @author mvk
 */
class S5LongBooleanIndexImpl
    extends S5AbstractIndex<Long, Boolean>
    implements ILongBooleanIndexEdit {
   ...
   */
  S5LongBooleanIndexImpl( boolean aUnique, long[] aKeys, boolean[] aValues, boolean aRestore ) {
  ...
  // ------------------------------------------------------------------------------------
  // Реализация ILongBooleanIndexEdit
  //
  @Override
  public int capacity() {
    return keys.length;
  ...
  @Override
  public void add( long[] aKeys, boolean[] aValues ) {
    TsNullArgumentRtException.checkNulls( aKeys, aValues );
    checkSize( aKeys.length, aValues.length );
  ...
  @Override
  public void add( Long aKey, Boolean aValue ) {
    TsNullArgumentRtException.checkNulls( aKey, aValue );
    add( aKey.intValue(), aValue.booleanValue() );
  }
  ...
  @Override
  public Boolean value( int aIndex ) {
    if( aIndex < 0 || aIndex >= watermark ) {
    ...
    return Boolean.valueOf( values[aIndex] );
  }

  @Override
  public boolean booleanValue( int aIndex ) {
    if( aIndex < 0 || aIndex >= watermark ) {
      Long valueIndex = Long.valueOf( aIndex );
      throw new TsIllegalArgumentRtException( MSG_ERR_INDEX_VALUE_OUT, valueIndex, Long.valueOf( watermark ) );
    }
  ...
  @Override
  public Boolean findValue( Long aKey ) {
    return Boolean.valueOf( findValue( aKey.longValue() ) );
  }
  ...
  @Override
  public Boolean getValue( Long aKey ) {
    return Boolean.valueOf( getValue( aKey.longValue() ) );
  }
