...
import org.toxsoft.uskat.s5.utils.indexes.ILongByteIndexEdit;

/**
 * Бинарный индекс байтового значения по длинному целочисленному ключу
 *
 * @author mvk
 */
class S5LongByteIndexImpl
    extends S5AbstractIndex<Long, Byte>
    implements ILongByteIndexEdit {
   ...
   */
  S5LongByteIndexImpl( boolean aUnique, long[] aKeys, byte[] aValues, boolean aRestore ) {
  ...
  @Override
  public void add( long[] aKeys, byte[] aValues ) {
    TsNullArgumentRtException.checkNulls( aKeys, aValues );
    checkSize( aKeys.length, aValues.length );
  ...
  @Override
  public void add( Long aKey, Byte aValue ) {
    TsNullArgumentRtException.checkNulls( aKey, aValue );
    add( aKey.intValue(), aValue.byteValue() );
  }
  ...
  public Byte value( int aIndex ) {
    ...
    return Byte.valueOf( values[aIndex] );
  }

  @Override
  public byte byteValue( int aIndex ) {
    if( aIndex < 0 || aIndex >= watermark ) {
      Long valueIndex = Long.valueOf( aIndex );
      throw new TsIllegalArgumentRtException( MSG_ERR_INDEX_VALUE_OUT, valueIndex, Long.valueOf( watermark ) );
    }
  ...
  @Override
  public Byte findValue( Long aKey ) {
    return Byte.valueOf( findValue( aKey.longValue() ) );
  }
  ...
  @Override
  public Byte getValue( Long aKey ) {
    return Byte.valueOf( getValue( aKey.longValue() ) );
  }
