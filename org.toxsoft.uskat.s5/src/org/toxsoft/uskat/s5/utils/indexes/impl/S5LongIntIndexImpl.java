...
import org.toxsoft.uskat.s5.utils.indexes.ILongIntIndexEdit;

/**
 * Бинарный индекс целочисленного значения по длинному целочисленному ключу
 *
 * @author mvk
 */
class S5LongIntIndexImpl
    extends S5AbstractIndex<Long, Integer>
    implements ILongIntIndexEdit {
   ...
   */
  S5LongIntIndexImpl( boolean aUnique, long[] aKeys, int[] aValues, boolean aRestore ) {
  ...
  @Override
  public void add( ILongList aKeys, IIntList aValues ) {
    TsNullArgumentRtException.checkNulls( aKeys, aValues );
  ...
  @Override
  public void add( long[] aKeys, int[] aValues ) {
    TsNullArgumentRtException.checkNulls( aKeys, aValues );
    checkSize( aKeys.length, aValues.length );
  ...
  @Override
  public Integer findValue( Long aKey ) {
    return Integer.valueOf( findValue( aKey.longValue() ) );
  }

  @Override
  public int findValue( long aKey ) {
    int index = findIndex( aKey );
    if( index < 0 ) {
  ...
  @Override
  public Integer getValue( Long aKey ) {
    return Integer.valueOf( getValue( aKey.longValue() ) );
  }
