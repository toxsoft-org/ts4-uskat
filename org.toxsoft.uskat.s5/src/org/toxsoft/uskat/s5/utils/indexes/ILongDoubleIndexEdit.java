...
/**
 * Индекс вещественного(double) значения по длинному целочисленному ключу с возможностью редактирования
 *
 * @author mvk
 */
public interface ILongDoubleIndexEdit
    extends ILongDoubleIndex, IIndexEdit<Long, Double> {

  /**
   * Добавляет в индекс указанный массив ключей и их значений
   *
   * @param aKeys long[] ключи индекса
   * @param aValues double[] значения индекса
   ...
   */
  void add( long[] aKeys, double[] aValues );

  /**
   * Добавляет в индекс ключ и его значение
   *
   * @param aKey long ключ
   * @param aValue double значение ключа
   * @throws TsInternalErrorRtException переполнение индекса
   */
  void add( long aKey, double aValue );
}
