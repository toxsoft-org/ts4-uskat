package org.toxsoft.uskat.s5.server.backend.supports.histdata.sequences.sync;

import static org.toxsoft.core.tslib.av.impl.AvUtils.*;
import static org.toxsoft.uskat.s5.server.backend.supports.histdata.sequences.S5HistDataSequenceFactory.*;
import static org.toxsoft.uskat.s5.server.backend.supports.histdata.sequences.sync.IS5Resources.*;
...
import org.toxsoft.uskat.s5.server.backend.supports.histdata.sequences.S5HistDataSyncBlock;
 ...
 */
@Entity
public class S5HistDataSyncFloatingEntity
    extends S5HistDataSyncBlock<double[], S5HistDataSyncFloatingBlobEntity> {
   ...
   * @return {@link S5HistDataSyncFloatingEntity} блок значений
   ...
   */
  static S5HistDataSyncFloatingEntity create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataSyncFloatingBlobEntity blob = new S5HistDataSyncFloatingBlobEntity( values );
    return new S5HistDataSyncFloatingEntity( aTypeInfo, aGwid, startTime, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataSyncFloatingEntity() {
  }
   ...
   * @param aBlob {@link S5HistDataSyncFloatingBlobEntity} реализация blob-а в котором хранятся значения блока
   * @throws TsNullArgumentRtException любой аргумент = null
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   * @throws TsIllegalArgumentRtException описание данного должно представлять синхронное данное
   */
  S5HistDataSyncFloatingEntity( IParameterized aTypeInfo, Gwid aGwid, long aStartTime,
      S5HistDataSyncFloatingBlobEntity aBlob ) {
   ...
   */
  S5HistDataSyncFloatingEntity( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  protected ISequenceBlockEdit<ITemporalAtomicValue> doCreateBlock( IParameterized aTypeInfo, long aStartTime,
      double[] aValues ) {
    S5HistDataSyncFloatingBlobEntity blob = new S5HistDataSyncFloatingBlobEntity( aValues );
    return new S5HistDataSyncFloatingEntity( aTypeInfo, gwid(), aStartTime, blob );
  }

  @Override
  protected S5HistDataSyncFloatingBlobEntity doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataSyncFloatingBlobEntity( aResultSet );
  }
