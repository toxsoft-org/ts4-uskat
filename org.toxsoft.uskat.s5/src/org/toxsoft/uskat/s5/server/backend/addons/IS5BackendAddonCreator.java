...
public interface IS5BackendAddonCreator
   ...
   * <p>
   * Процессоры сообщений могут(!) использоваться для "накопления" данных перед их фактической отправкой. Например,
   * передача значений текущих данных может осуществляться не чаще одного раза в секунду (параметр настройки).
   *
