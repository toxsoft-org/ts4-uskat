...
import static org.toxsoft.uskat.s5.server.backend.supports.histdata.sequences.async.IS5Resources.*;
 ...
 */
@Entity
public class S5HistDataAsyncIntegerEntity
    extends S5HistDataAsyncBlock<long[], S5HistDataAsyncIntegerBlobEntity> {
   ...
   * @return {@link S5HistDataAsyncIntegerEntity} блок значений
   ...
   */
  static S5HistDataAsyncIntegerEntity create( IParameterized aTypeInfo, Gwid aGwid,
    ...
    S5HistDataAsyncIntegerBlobEntity blob = new S5HistDataAsyncIntegerBlobEntity( timestamps, values );
    return new S5HistDataAsyncIntegerEntity( aTypeInfo, aGwid, blob );
  }

  /**
   * Конструктор без параметров (для JPA)
   */
  protected S5HistDataAsyncIntegerEntity() {
  }
   ...
   * @param aTypeInfo {@link IParameterized} параметризованное описание типа данного
   * @param aGwid {@link Gwid} НЕабстрактный {@link Gwid}-идентификатор данного
   * @param aBlob {@link S5HistDataAsyncIntegerBlobEntity} реализация blob-а в котором хранятся значения блока и их
   *          метки времени
   * @throws TsIllegalArgumentRtException количество значений в блоке = 0
   ...
   * @throws TsIllegalArgumentRtException описание данного должно представлять асинхронное данное
   */
  S5HistDataAsyncIntegerEntity( IParameterized aTypeInfo, Gwid aGwid, S5HistDataAsyncIntegerBlobEntity aBlob ) {
   ...
   */
  S5HistDataAsyncIntegerEntity( ResultSet aResultSet ) {
    super( aResultSet );
  }
  ...
  @Override
  protected S5HistDataAsyncIntegerBlobEntity doCreateBlob( ResultSet aResultSet ) {
    return new S5HistDataAsyncIntegerBlobEntity( aResultSet );
  }
